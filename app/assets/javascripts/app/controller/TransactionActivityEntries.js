Ext.define('AM.controller.TransactionActivityEntries', {
  extend: 'Ext.app.Controller',

  stores: ['TransactionActivities', 'TransactionActivityEntries'],
  models: ['TransactionActivityEntry'],

  views: [
    'accounting.transactionactivityentry.List',
    'accounting.transactionactivityentry.Form',
  ...
  init: function() {
    this.control({
      'transactionactivityentrylist': {
        itemdblclick: this.editObject,
        selectionchange: this.selectionChange ,
				afterrender : this.loadObjectList
      },
      'transactionactivityentryform button[action=save]': {
        click: this.updateObject
      },

	 
      'transactionactivityentrylist button[action=addObject]': {
        click: this.addObject
      },

			'transactionactivityentrylist button[action=repeatObject]': {
        click: this.addObject
      },


      'transactionactivityentrylist button[action=editObject]': {
        click: this.editObject
      },
      'transactionactivityentrylist button[action=deleteObject]': {
	...
	// 	var store = this.getTransactionActivityEStore();
	// 	
	// 	store.load({
	// 		params : {
  ...
  addObject: function(button) {
		...
		var widgetName = 'transactionactivityentryform'; 
  ...
  updateObject: function(button) {
    ...
    var values = form.getValues();
		...
		if( record ){
			...
			record.save({
				params : {
					transaction_activity_id : parentRecord.get('id')
				},
    ...
    var record = this.getList().getSelectedObject();
		if(!record){return;}
		var parent_id = record.get('transaction_activity_id');
		var list  = this.getList();
    ...
    if (record) {
			record.destroy({
				success : function(record){
					...
					list.getStore().load({
						params : {
							transaction_activity_id : parent_id
						}
					});
				},
				failure : function(record,op ){
